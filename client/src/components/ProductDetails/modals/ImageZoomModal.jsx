...
const ImageZoomModal = ({
  ...
  const normalizeImageUrl = (url) => {
    ...
    return `https://www.smitox.com/${cleanUrl}`;
  };

  const getMainImageUrl = () => {
    if (selectedImage === 0) {
      return normalizeImageUrl(product.photos);
    }
    if (product.multipleimages && Array.isArray(product.multipleimages) &&
      product.multipleimages.length > 0 && selectedImage <= product.multipleimages.length) {
      return normalizeImageUrl(product.multipleimages[selectedImage - 1]);
    }
    return normalizeImageUrl(product.photos);
  };
    ...
    <div style={{
      ...
      {(product.multipleimages && Array.isArray(product.multipleimages) && product.multipleimages.length > 0) && (
        <div style={{
          ...
          >
            <img
              src={normalizeImageUrl(product.photos)}
